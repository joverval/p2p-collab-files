/**
 * E2E Test I.1: STUN-only, direct connection.
 *
 * Scenario:
 ...
 *   5. Verify getRoute() returns a non-empty string (direct connection)
 *
 * Note: ICE_MODE is set via VITE_ICE_MODE env var at build time, not test runtime.
 * This test assumes the app was built with VITE_ICE_MODE=stun-only.
 */
import { test } from '@playwright/test';
import { createE2EHelpers } from '../helpers/e2e-helpers';
import { CONNECTION_TIMEOUT, ICE_MODE } from '../helpers/test-constants';

test('STUN-only direct connection — host + peer connect and route is reported', async ({
  browser,
}) => {
